...
function row(item){const examples=item.examples?.length?`<p class="examples"><span>武器例：</span>${item.examples.map(escape).join(' / ')}</p>`:'';return `<div class="role-row"><div><h4>${escape(item.name)}</h4><p>${escape(item.description||'')}</p>${examples}</div><div class="role-rating"><span aria-label="${item.score===null?'不明':item.score+' / 5'}">${rating(item)}</span><small>${item.score===null?'根拠不足':item.score+' / 5'}</small></div><p class="reason">${escape(item.reason)}</p></div>`;}
...
function showResult(){
 ...
 const selectedExamples=r.loadout.flatMap(loadout=>r.weapons.filter(weapon=>loadout.includes(weapon.name))).map(weapon=>`<p class="example-line"><strong>${escape(weapon.name)}</strong>：${weapon.examples.map(escape).join(' / ')}</p>`).join('');
 ...
 ${section('weapons','03','武器カテゴリ適性',r.weapons.map(row).join('')+`<div class="loadout"><p class="eyebrow">LOADOUT</p>${r.loadout.map((x,i)=>`<p>${i===0?'第一候補':'第二候補'}：<strong>${escape(x)}</strong></p>`).join('')||'<p>武器構成は保留。必要な能力を確認してから選びましょう。</p>'}${selectedExamples?`<div class="example-list"><p class="muted">具体的な武器名の例（環境や好みで入れ替え可）</p>${selectedExamples}</div>`:''}<p class="muted">評価3以上のARを軸に、適性のある補助武器を組み合わせています。各カテゴリの最新性能・入手条件は変わるため、例は候補として見てください。</p></div><h3>交戦距離の目安</h3><div class="distances">${r.distances.map((d,i)=>`<div class="distance"><strong>${d.name}</strong>${meter(d.score)}<p>${d.score===null?'情報不足':d.score+' / 5'}<br>${['遭遇・建物内','ARでの継続射撃','精密射撃・狙撃'][i]}</p></div>`).join('')}</div>`)}
