...
function toText(r,name=''){
 ...
 section('武器カテゴリ適性',r.weapons.map(a=>`${a.name}：${rate(a)} / ${a.reason}${a.examples?.length?`\n武器例：${a.examples.join(' / ')}`:''}`)),
 section('武器構成の候補',r.loadout.length?[...r.loadout,...r.loadout.flatMap(loadout=>r.weapons.filter(weapon=>loadout.includes(weapon.name)).map(weapon=>`${weapon.name}の例：${weapon.examples.join(' / ')}`))]:['情報・適性の確認が必要なため保留。']),
